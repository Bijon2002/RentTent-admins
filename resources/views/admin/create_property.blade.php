...
@section('title', 'Create Property - RentTent Admin')
...
<div class="create-form-section">
    ...
    <form method="POST" action="{{ route('admin.properties.store') }}" class="create-form" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="number" name="monthly_rent" id="monthly_rent" step="0.01" min="0" value="{{ old('monthly_rent') }}" required>
            </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <select name="police_zone_rating" id="police_zone_rating" required>
                    ...
                    <option value="1" {{ old('police_zone_rating') == '1' ? 'selected' : '' }}>1 Star</option>
                    <option value="2" {{ old('police_zone_rating') == '2' ? 'selected' : '' }}>2 Stars</option>
                    <option value="3" {{ old('police_zone_rating') == '3' ? 'selected' : '' }}>3 Stars</option>
                    <option value="4" {{ old('police_zone_rating') == '4' ? 'selected' : '' }}>4 Stars</option>
                    <option value="5" {{ old('police_zone_rating') == '5' ? 'selected' : '' }}>5 Stars</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" name="posted_date" id="posted_date" value="{{ old('posted_date') }}" required>
            </div>

            <div class="form-group">
                <label for="is_food_included">Food Included</label>
                <select name="is_food_included" id="is_food_included">
                    <option value="0" {{ old('is_food_included') == '0' ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('is_food_included') == '1' ? 'selected' : '' }}>Yes</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <small class="form-text">Upload multiple photos for the property (optional)</small>
        </div>

        <div class="form-group">
            <label for="is_approved">Approval Status</label>
            <select name="is_approved" id="is_approved">
                <option value="0" {{ old('is_approved') == '0' ? 'selected' : '' }}>Pending</option>
                <option value="1" {{ old('is_approved') == '1' ? 'selected' : '' }}>Approved</option>
            </select>
        </div>
